 ...
 */
 var countNodes = function(root) {
  ...
  const heigh = function(node){
    if(!node) return 0
      if(!node.left && !node.right) return 1
      return 1 + heigh(node.left)
  }

  const lh = heigh(root.left)
  const rh = heigh(root.right)
  ...
  if(lh > rh){
    ans = rh > 0 ? count(root.left) + 1 : 2<<(rh - 1)
  }else{
    ans = lh > 0 ? count(root.right) + 1: 2<<(lh - 1)
  }
